...
**A golden-set label that turned out to be wrong.** I wrote a not-established case claiming Claude Code can split a large refactor into multiple pull requests based on a file dependency graph, expecting no evidence either way. I was wrong: Claude Code has a real `/batch` skill that splits one large change into 5 to 30 worktree-isolated subagents, each opening its own pull request. The system found this, at 0.55 confidence, and returned confirmed. But the citation judge flagged the citation as implausible, and correctly so: `/batch` splits work across subagents, not by a file dependency graph, so the claim's specific mechanism isn't established even though the general shape of the feature is real. The system's own confidence score (0.55, below the 0.7 threshold) routed this to human review regardless of the verdict it landed on, which is the behavior that matters here: a low-confidence match, even a nominally confirmed one, gets sent to a person rather than asserted outright. The bug, if there is one, is in my case-writing, not the pipeline. I hadn't read the corpus closely enough before I decided this was unestablished.
